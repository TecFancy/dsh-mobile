...
function observeFrameState(frame, callback) {
	const read = () => {
		...
		const detailsCollapsed = frame.hasAttribute("data-details-collapsed") || frame.hasAttribute("data-rightbar-collapsed");
...
//#endregion
//#region src/client/focus-guard.ts
/**
...
const isComposerEditable = (el) => el instanceof HTMLElement && el.matches("textarea, input, [contenteditable]") && el.closest(COMPOSER_SLOT) !== null;
/**
...
*/
function installMobileFocusGuard(doc) {
	...
	const onFocusIn = (event) => {
		...
		if (isComposerEditable(event.target)) event.target.blur();
	};
	const blurComposerIfFocused = () => {
		if (userTouched) return;
		if (isComposerEditable(doc.activeElement)) doc.activeElement.blur();
	};
	const observer = new MutationObserver(() => {
		if (doc.body.hasAttribute("data-dsh-mobile")) blurComposerIfFocused();
	});
	observer.observe(doc.body, {
		attributes: true,
		attributeFilter: ["data-dsh-mobile"]
	});
